/**
 * AUTO EMAIL REPORTS ENGINE (State Machine Architecture)
 * ----------------------------------------------------------------------------
 * Splits heavy PDF generation into micro-tasks, temporarily caches files in 
 * Google Drive, and uses PropertiesService to track exact mid-customer progress,
 * completely neutralizing the 6-minute Google Apps Script timeout limit.
 */

const CONFIG = {
  TAB_RDS: 'Jaquar RDs',
  COL_STATUS: 8, 
  MAX_EXECUTION_TIME_MS: 3 * 60 * 1000, // 3 minutes! Guarantees we never start a long task with little time left.
  RESUME_TRIGGER_MINS: 2 
};

// ---- MAIN ENTRY POINT (Run this daily via Trigger) ----
async function startDailyEmailBatch() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sh = ss.getSheetByName(CONFIG.TAB_RDS);
  
  // 1. Clear visual statuses
  const lastRow = sh.getLastRow();
  if (lastRow > 1) {
    sh.getRange(2, CONFIG.COL_STATUS, lastRow - 1, 1).clearContent();
  }
  
  // 2. Wipe the hidden mid-process states from yesterday
  const props = PropertiesService.getDocumentProperties();
  const keys = props.getKeys();
  keys.forEach(k => {
    if (k.startsWith('BATCH_')) props.deleteProperty(k);
  });

  // 3. NEW: Automatically empty the Temp Folder of any orphaned files from yesterday
  const folders = DriveApp.getFoldersByName("Temp_Reports_AutoEmail");
  if (folders.hasNext()) {
    const folder = folders.next();
    const files = folder.getFiles();
    while (files.hasNext()) {
      files.next().setTrashed(true);
    }
  }
  
  // Kick off processing
  await processEmailBatch();
}

// ---- CORE PROCESSOR ----
async function processEmailBatch() {
  const startTime = new Date().getTime();
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sh = ss.getSheetByName(CONFIG.TAB_RDS);
  const data = sh.getDataRange().getValues(); 
  const tempFolder = getTempFolder();
  const props = PropertiesService.getDocumentProperties();
  
  // Loop starts at 1 to skip the header row
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const customer = String(row[0]).trim(); 
    const email1 = String(row[4]).trim();   
    const email2 = String(row[5]).trim();   
    const email3 = String(row[6]).trim();   
    
    if (!customer || !email1) continue;

    // Load hidden state for this specific row
    const stateKey = `BATCH_${i}`;
    let stateStr = props.getProperty(stateKey);
    let state = stateStr ? JSON.parse(stateStr) : { status: 'Pending' };

    if (state.status === 'Sent' || state.status === 'Error') continue;

    Logger.log(`Processing: ${customer} | Phase: ${state.status}`);

    try {
      // ==========================================
      // PHASE 1: AGING PDF & DATA EXTRACTION
      // ==========================================
      if (state.status === 'Pending') {
        runAgingForRow(ss, i + 1); 
        SpreadsheetApp.flush();
        
        const agingSheet = ss.getSheetByName('Aging Report');
        const totalOutstanding = agingSheet.getRange('I5').getDisplayValue() || '0.00';
        const totalOverdue = agingSheet.getRange('H9').getDisplayValue() || '0.00';

        const agingBlobs = makeAgingPdfV3(ss); 
        const agingPdf = await PDFApp.mergePDFs(agingBlobs);
        agingPdf.setName(`Aging_Report_${customer.replace(/[^A-Za-z0-9]+/g, '_')}.pdf`);
        
        // Save to Drive and record state
        const file = tempFolder.createFile(agingPdf);
        state.agingId = file.getId();
        state.outstanding = totalOutstanding;
        state.overdue = totalOverdue;
        state.status = 'Aging_Done';
        
        props.setProperty(stateKey, JSON.stringify(state));
        sh.getRange(i + 1, CONFIG.COL_STATUS).setValue('Generating PDFs...');

        if (checkTimeout(startTime)) return; 
      }

      // ==========================================
      // PHASE 2: STATEMENT PDF
      // ==========================================
      if (state.status === 'Aging_Done') {
        const stmtSheet = ss.getSheetByName('Statement');
        stmtSheet.getRange('B1').setValue(customer);
        generateStatement(); 
        SpreadsheetApp.flush();
        
        const stmtBlobs = makeStatementPdfV2(ss); 
        const stmtPdf = await PDFApp.mergePDFs(stmtBlobs);
        stmtPdf.setName(`Statement_${customer.replace(/[^A-Za-z0-9]+/g, '_')}.pdf`);

        // Save to Drive and record state
        const file = tempFolder.createFile(stmtPdf);
        state.stmtId = file.getId();
        state.status = 'Statement_Done';
        
        props.setProperty(stateKey, JSON.stringify(state));

        if (checkTimeout(startTime)) return; 
      }

      // ==========================================
      // PHASE 3: COMPILE, EMAIL, AND CLEANUP
      // ==========================================
      if (state.status === 'Statement_Done') {
        const agingFile = DriveApp.getFileById(state.agingId);
        const stmtFile = DriveApp.getFileById(state.stmtId);

        const ccSet = new Set();
        if (email2 && email2 !== email1) ccSet.add(email2);
        if (email3 && email3 !== email1 && email3 !== email2) ccSet.add(email3);
        const ccEmailStr = Array.from(ccSet).join(',');

        const today = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'dd-MM-yyyy');
        const subject = `${customer} - Account Statement & Aging (${today})`;
        
        const htmlBody = `
          <div style="font-family: Arial, sans-serif; color: #333333; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
            <div style="background-color: #283C50; color: #ffffff; padding: 20px; text-align: center;">
              <h2 style="margin: 0; font-size: 22px; letter-spacing: 1px;">BHARAT GLASS HOUSE</h2>
              <p style="margin: 5px 0 0; font-size: 13px; color: #c9d3dd; text-transform: uppercase;">Account Statement & Aging Report</p>
            </div>
            <div style="padding: 30px 20px;">
              <p style="font-size: 15px; margin-top: 0;">Dear <strong>${customer}</strong>,</p>
              <p style="font-size: 14px; line-height: 1.6;">Please find attached your updated Account Statement and Aging Report as of <strong>${today}</strong>.</p>
              
              <div style="background-color: #f8f9fa; border: 1px solid #e9ecef; border-radius: 6px; padding: 15px; margin: 25px 0; display: table; width: 100%; box-sizing: border-box;">
                <div style="display: table-cell; width: 50%; text-align: center; border-right: 1px solid #dee2e6; padding-right: 10px;">
                  <p style="margin: 0; font-size: 12px; color: #6c757d; text-transform: uppercase;">Total Outstanding</p>
                  <p style="margin: 5px 0 0; font-size: 18px; color: #283C50; font-weight: bold;">₹ ${state.outstanding}</p>
                </div>
                <div style="display: table-cell; width: 50%; text-align: center; padding-left: 10px;">
                  <p style="margin: 0; font-size: 12px; color: #6c757d; text-transform: uppercase;">Total Overdue</p>
                  <p style="margin: 5px 0 0; font-size: 18px; color: #d32f2f; font-weight: bold;">₹ ${state.overdue}</p>
                </div>
              </div>

              <div style="background-color: #fff8eb; border-left: 4px solid #f5b041; padding: 12px 15px; margin: 20px 0;">
                <p style="margin: 0; font-size: 13px; color: #5c4011;"><strong>Action Required:</strong> Kindly review the attached documents. If you observe any discrepancies, please report them to us within <strong>7 days</strong> so we can reconcile our records.</p>
              </div>

              <p style="font-size: 14px; line-height: 1.6;">If you have any questions or require further clarification regarding specific invoices, please reply directly to this email.</p>
            </div>
            <div style="background-color: #f1f5f9; padding: 20px; font-size: 11px; color: #666666; text-align: center; border-top: 1px solid #e0e0e0;">
              <p style="margin: 0; font-size: 12px;"><strong>Accounts Team</strong> | BHARAT GLASS HOUSE</p>
              <p style="margin: 5px 0 0;">834/A/1 Amrik Singh Road, Opp. Annapurna Mandir, Bathinda</p>
              <p style="margin: 5px 0 0;">GSTIN: 03AAAFB7505P1ZI</p>
            </div>
          </div>
        `;

        MailApp.sendEmail({
          to: email1, // Fixed: uses the correct variable
          cc: ccEmailStr,
          subject: subject,
          htmlBody: htmlBody,
          attachments: [agingFile.getBlob(), stmtFile.getBlob()]
        });

        // Cleanup: Delete temp PDFs from Drive
        agingFile.setTrashed(true);
        stmtFile.setTrashed(true);

        state.status = 'Sent';
        props.setProperty(stateKey, JSON.stringify(state));
        sh.getRange(i + 1, CONFIG.COL_STATUS).setValue('Sent');
        
        Logger.log(`Email sent successfully to ${email1}`); // Fixed

        if (checkTimeout(startTime)) return; 
      }

    } catch (e) {
      Logger.log(`Error processing ${customer}: ${e.message}`);
      sh.getRange(i + 1, CONFIG.COL_STATUS).setValue('Error');
      
      state.status = 'Error';
      props.setProperty(stateKey, JSON.stringify(state));
    }
  }
  
  Logger.log('Entire batch completed successfully!');
}

// ---- HELPERS ----

function checkTimeout(startTime) {
  if (new Date().getTime() - startTime > CONFIG.MAX_EXECUTION_TIME_MS) {
    Logger.log('Time limit reached. Pausing and scheduling resume trigger...');
    deleteContinuationTriggers();
    ScriptApp.newTrigger('processEmailBatch')
      .timeBased()
      .after(CONFIG.RESUME_TRIGGER_MINS * 60 * 1000)
      .create();
    return true; 
  }
  return false;
}

function deleteContinuationTriggers() {
  const triggers = ScriptApp.getProjectTriggers();
  for (const trigger of triggers) {
    if (trigger.getHandlerFunction() === 'processEmailBatch') {
      ScriptApp.deleteTrigger(trigger);
    }
  }
}

function getTempFolder() {
  const folderName = "Temp_Reports_AutoEmail";
  const folders = DriveApp.getFoldersByName(folderName);
  if (folders.hasNext()) {
    return folders.next();
  }
  return DriveApp.createFolder(folderName);
}